...
from Dataset_Loader import TripletFaceDataset
import torch
import torch.optim as optim
from FaceMatch import FaceEmb
import os
...
if __name__ == "__main__":
    ...
    train_dataset = TripletFaceDataset(config['path_train_set'])
    print(f"Train dataset size: {len(train_dataset)}")

    trainloader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size,
                                          shuffle=True, num_workers=2)
    # Build the testing Data loader
    test_dataset = TripletFaceDataset(config['path_train_set'])
